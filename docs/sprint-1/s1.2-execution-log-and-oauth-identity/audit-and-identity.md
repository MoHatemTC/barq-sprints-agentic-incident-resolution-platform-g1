...
- **Repository secret scan (`scripts/scan_repo_secrets.py`, S1.2 `CRED-01`, #51)** — runs in CI through `tests/repo/test_no_secrets_committed.py`, locally as `uv run python scripts/scan_repo_secrets.py`. Exactly what it scans:
  - Every file reported by `git ls-files` — the tracked set, so the result does not depend on what else sits on the machine running it — with **no extension filter**. `.env` content, `.py`, `.ts`, `.cjs`, `.sh`, `.xml`, `.md`, `.yaml`, `.yml`, `.json`, `.txt`, `.toml`, `.ini` and extensionless files are all read as UTF-8 text. Files that do not decode as UTF-8 (images, archives) are skipped and reported as skipped. Files other checks leave out — including `scripts/verification_report.json` and this scanner's own allowlist — are scanned, not excluded.
  - Two rule families: **token shapes** (AWS access key ids, GitHub, Slack, OpenAI-style and Stripe keys, Google API keys, PEM private-key blocks, JWTs) wherever they appear, and **credential assignments** — a name containing `password`/`passwd`/`pwd`/`secret`/`token`/`credential`/`api-key`/`access-key`/`private-key`/`signing-key`/`client-secret` followed by `=`, `:` or a `…</name>` tag, plus `curl -u user:password` and `wget --user`. An assigned value only counts when it looks like a credential: at least six characters, containing both letters and digits, and not a placeholder, template (`${VAR}`), path or URL.
  - Findings are printed as `file:line [rule] <name> = <redacted>`; the matched value is never printed, logged or stored.
  - Exemptions live in `scripts/secret_scan_allowlist.json` as `(file, rule, sha256-of-value, reason)`. No directory is exempt: the deliberately fake credential literals that masking tests assert are redacted are each listed individually with a written reason, so a new secret added anywhere in the tree still fails the check.
  - The command exits `1` on any finding, and `tests/repo/test_no_secrets_committed.py` fails the build on one, on an allowlist entry without a reason, and if the report or allowlist ever drops out of the scanned set.
