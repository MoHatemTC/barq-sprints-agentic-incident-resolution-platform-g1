...
- **Verification Status**: **22 / 22 PASS (100% Verified Empirical Compliance)**
...
|---|---|---|
| `x_2215032_ai_inc_0.integration_writer` | Scoped (`x_2215032_ai_inc_0`) | Grants write permission to permitted scoped incident AI fields (e.g. `ai_classification`, `ai_human_review_required`, suggestion, resolution) and internal `work_notes`. |
...
| `execution_id` | Execution ID | String (100) | Unique string (e.g. `exec_verify_<hex>`) | **Indexed unique trace key** for distributed tracing and single-record lookup. |
...
| `status` | Status | String (40) | `started`, `succeeded`, `failed`, `blocked`, `awaiting_approval`, `abandoned` | Durable execution lifecycle outcome choices. |
...
|    * incident.x_2215032_ai_inc_0_ai_human_review_required -> Human Triage Flag|
|    * (and scoped AI execution fields: suggestion, resolution, confidence, etc.)|
|                                                                                |
...
| `incident.x_2215032_ai_inc_0_ai_human_review_required` | **ALLOW** | `integration_writer` | Scoped boolean flag allowing the triage agent to flag complex or low-confidence incidents for manual operator inspection. |
...
#### Complete Test Run Results (22 of 22 Passed)
