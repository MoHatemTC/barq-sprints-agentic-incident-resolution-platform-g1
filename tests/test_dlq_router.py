...
async def test_replay_dlq_moves_event_from_dlq_to_events_queue(app_instance) -> None:
    """Ensure replay pops from DLQ and enqueues to incident events queue atomically."""
    import json
    ...
    mock_pipe.lpush.assert_called_once_with(
        INCIDENT_EVENTS_QUEUE, json.dumps({"number": "INC009", "sys_id": "sys999"})
    )
