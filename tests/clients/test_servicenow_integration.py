...
from app.exceptions.servicenow import ServiceNowAuthorizationError, ServiceNowError, ServiceNowValidationError
...
pytestmark = pytest.mark.skipif(
    not os.environ.get("SERVICENOW_TEST_INCIDENT_SYS_ID") or not os.environ.get("SERVICENOW_PASSWORD"),
...
async def test_live_acl_refusal(client: ServiceNowClient) -> None:
    ...
    # We attempt to patch sys_audit table directly which is restricted to prevent tampering
...
async def test_live_verify_log_entries_across_outcomes(client: ServiceNowClient, test_sys_id: str, status: ExecutionStatus) -> None:
    ...
    payload = ExecutionLogCreatePayload(
        ...
        error="Test error" if status in [ExecutionStatus.BLOCKED, ExecutionStatus.FAILED, ExecutionStatus.ABANDONED] else None
    )
    
    log_entry = await client.create_execution_log(payload)
    assert log_entry is not None
    assert log_entry.execution_id == exec_id
    assert log_entry.status == status

@pytest.mark.asyncio
async def test_live_execution_log_write_failure_safely_handled(client: ServiceNowClient, test_sys_id: str) -> None:
